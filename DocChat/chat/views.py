...
def get_conversation_fragments_without_memory(text):
    """
    Fetches conversation fragments without using any prior memory or context.

    Args:
    - text (str): The text representing the initial input or prompt for the AI.

    Returns:
    - conversation_fragments (dict): A dictionary containing conversation fragments generated by GPT-3.5.
          It includes the AI's responses and any contextual information inferred during the conversation.

    Description:
    This function utilizes the GPT-3.5 model to generate conversation fragments based solely on the provided input text.
    It initializes an instance of the ChatOpenAI class, configured with the 'gpt-3.5-turbo' model and the specified API key.
    The function sends the provided text as a message to the AI model, acting as a system role.
    The response received from GPT-3.5, which includes generated conversation fragments, is returned as a dictionary.

    Note:
    - This function doesn't incorporate any memory or context from previous interactions.
    - Ensure the 'OPENAI_API_KEY' environment variable is set with the appropriate API key.

    Example:
    ```python
    fragments = get_conversation_fragments_without_memory("Start of conversation...")
    # Output: {'messages': [{'role': 'system', 'content': 'Start of conversation...'}, ...]}
    ```
    """
    api_key = os.getenv("OPENAI_API_KEY")
    bot_gpt = ChatOpenAI(model_name="gpt-3.5-turbo", openai_api_key=api_key)
    conversation_fragments = bot_gpt.completion_with_retry(
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if text_chunks:
                ...
                response = get_conversation_fragments_without_memory(message_text)
